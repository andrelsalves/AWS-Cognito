# Roteiro da Implementaçao do AWS Cognito a uma API REST
...
- Protocol - REST -> Create new API -> API name [project_cognito] -> Endpoint Type - Regional -> Create API
- Resources -> Actions -> Create Resource -> Resource Name [items] -> Create Resource

### No Amazon DynamoDB

- DynamoDB Dashboard -> Tables -> Create table -> Table name [project_cognito] -> Partition key [id] -> Create table

### No AWS Lambda

#### Função para inserir item

- Lambda Dashboard -> Create function -> Name [project_cognito_put_items] -> Create function
...
- Review policy -> Name [project_cognito_policy] -> Create policy
...
- Cognito Dashboard -> Manage User Pools -> Create a User Pool -> Pool name [DioCognito]
...
- Which app clients will have access to this user pool? -> App client name [DioCog] -> Create App Client -> Next Step
- Create Pool
...
- Domain name -> Domain prefix [diocg] -> Save
...
- Name [DioCognitoAuth] -> Type - Cognito -> Cognito User Pool [pool criada anteriormente] -> Token Source [Authorization]
...
- Auth URL [https://diocg.auth.us-east-1.amazoncognito.com/login]
